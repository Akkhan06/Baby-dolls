...
const Category = () => {
    ...
    useEffect(() => {
      fetch('http://localhost:5000/subcategory_all')
      .then(res => res.json())
      .then(data => setSubData(data))
    },[])

    const filterItems = id => {
      const items = subData.filter(x => x.categoryId == id)
      setSubData2(items)
    }
...
<h1 className="text-gray-500 my-6">Lorem ipsum dolor sit amet consectetur, adipisicing elit. Ipsum, iste.</h1>
      ...
      <TabList className={'flex justify-center tabs gap-9 tabs-boxed mb-10 bg-warning font-semibold '}>
        <Tab onClick={() => filterItems('1')} className="tab text-white" style={{ backgroundColor: selectedTab === 0 ? 'gray' : '' }}>Cuddly Soft Dolls</Tab>
        <Tab onClick={() => filterItems('2')}  className="tab text-white" style={{ backgroundColor: selectedTab === 1 ? 'gray' : '' }}>Animal Friends Dolls</Tab>
        <Tab onClick={() => filterItems('3')} className="tab text-white" style={{ backgroundColor: selectedTab === 2 ? 'gray' : '' }}>Adorable Baby Dolls</Tab>
      </TabList>
